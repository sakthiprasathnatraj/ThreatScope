...
public class TrafficClassifier {
    ...
    public static TrafficClass classify(String sourceIP, ThreatType threatType, int portCount, long duration) {
        ...
        // Backdoor attempts are highly malicious
        if (threatType == ThreatType.BACKDOOR_ATTEMPT) {
            return TrafficClass.CONFIRMED_THREAT;
        }

        // Suspicious patterns (Null/XMAS scans) warrant monitoring
        if (threatType == ThreatType.SUSPICIOUS_PATTERN) {
